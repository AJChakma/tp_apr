...
class Source(object):
    ...
    def run(self):
        ...
        while True:
            ...
            self.packet_list.append(generated_packet)
            if self.debug:
...
if __name__=="__main__":
    ...
    for i in range(nb_simulations):
        ...
        #Packet drop ratio
        dropped_packets = [x for x in src1.packet_list + src2.packet_list if x.dropped]
        sent_packets = [x for x in src1.packet_list + src2.packet_list if not x.dropped]
#        packet_drop_ratio1.append(len([x for x in dropped_packets if x.source == "Router1"])/len([x for x in sent_packets if x.source == "Router1"]))
#        packet_drop_ratio2.append(len([x for x in dropped_packets if x.source == "Router2"])/len([x for x in sent_packets if x.source == "Router2"]))
#        packet_drop_ratio3.append(len([x for x in dropped_packets if x.source == "Channel"])/len([x for x in sent_packets if x.source == "Channel"]))
